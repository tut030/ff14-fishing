...
COIN_SEALS = 200              # ★探险币单价 军票/枚(FF14同款) [待砍]
SEAL_BASE = 200               # ★换票保底(最低档旧装备也值几百票) [待砍]
SEAL_PER_LV = 18              # ★每级加成: Lv50装≈1100票, Lv100装≈2000票 [待砍]
...
# 探险经验倍率(基准=wiki官方经验过 leveling._compress 同一台压缩机) [待砍]
...
ARM_GIL_PER_LV = 30          # ★武器价 = max(300, 等级×30)±10% [待砍]
MEMCARD_P = {"short": 0.06, "long": 0.15, "free": 0.25}   # ★内存卡概率 [待砍]
...
_PITCH = ("🏷 雇员中介所(市场板隔壁)。琪琪茹坐在三层坐垫上, 把一份契约推到你面前:\n"
          ...
          "   形态: 报一只萌感名录里的名字, 它就以那个样子来上班(魔兽形态)\n"
...
def hire(state: dict, arg: str = "") -> str:
    ...
    if form_raw in _RACE_ALIAS:
        ...
        return (f"🏷 琪琪茹对着名录找了半天:\"没有「{form_raw}」这种形态。\"\n"
                "   八族: " + "/".join(RACES) + "\n"
                "   魔兽形态请报萌感名录里的官方中文名(docs/宠物名录索引.md 有全表)")
...
def _dex(state: dict, arg: str) -> str:
    ...
    a = (arg or "").strip()
    if a and not a.isdecimal():                      # 按名字看详情
        lid = _MOE_BY_NAME.get(a)
        if not lid or lid not in owned:
            return f"名录里还没有「{a}」——多派探险, 惊喜会自己来。"
        e = MOE[lid]
