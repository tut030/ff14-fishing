...
- 🎪 **金碟萌宠大赛**：带召唤中的跟宠登台，三轮（亮相/才艺/自由发挥）各四选一（稳健卖萌/大胆炫技/剑走偏锋/交给崽自己拿主意），与四位对手拼名次赢 MGP；平日 `pet` 摸摸、`pet treat` 投喂悄悄攒默契，上场有加分，冠军累计解锁限定称号
...
- 📦 **单文件随身版 & 存档串**：`tools/build_onefile.py` 打出零依赖单文件 `ff14_fishing.py`，传给能跑代码的 AI 即玩；`export`/`import` 把整份进度变成一段可复制文字，沙箱重置不丢档，还能发给朋友接力
...
│   └── contest.py       金碟萌宠大赛（v43：跟宠三轮·默契加分·名次奖）
├── data/              静态数据
│   ├── fish.json             2119 条鱼
│   ├── ocean.json            259 条海钓鱼
│   ├── weather.json          天气概率表
...
```bash
python tools/build_onefile.py     # 在项目根目录生成 ff14_fishing.py（约 715 KB, 版本号取自 CHANGELOG）
```

把 `ff14_fishing.py` 这**一个文件**传给能跑代码的 AI（ChatGPT 代码解释器 / claude.ai / Claude Code），她这样玩：

```python
import ff14_fishing as g
print(g.guide())                              # 先读速通说明书
print(g.cmd("look", player="sakura"))
print(g.cmd("cast 10; sell all; look", player="sakura"))
```

命令行同样可用：`python ff14_fishing.py sakura "cast 10"`。
首次运行会在文件旁解出 `ff14_fishing_home/`（引擎+数据+存档目录）；升级版本重解引擎与数据，绝不动 `saves/`。

**运行环境每次对话会清空文件？（ChatGPT / 新对话沙箱）**
离开前发 `export`，游戏吐出一段「存档串」，整段复制自己存好；下次 `import <存档串>` 原地恢复。
串带 CRC 校验——复制缺一截会明确报错、原档不动；`import` 覆盖前还会把当前档另存 `<名>.pre_import.json`，手滑可回。
存档串只含游戏进度，没有任何密钥或个人信息，也可以发给朋友让她的 AI 接着玩你的档。
（能联网又能跑代码的 AI 还可以更省事：把仓库链接发给她，让她自己拉下来跑 `ai_play.py`。）
...
python3 -m pytest tests/ -v          # 148 个测试，1 秒内跑完
```
...
- `note` 给自己的备忘录: 跨 session 待办, 隔久了登录开场自动展示——念想不断线
- `bottles` 瓶中信: 钓鱼时偶尔捞到克克(联合开发)写的十二封信, 集齐向; 岸边的银发钓客也是她
